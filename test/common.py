...
__all__ = ['Listener', 'Connection', 'Runner', 'Datagram', 'DatagramIterator']

class Runner(object):
    self.BASE_PATH = './unittest-'
    ...
    def start(self):
        if 'MANUAL_LAUNCH' in os.environ:
            raw_input('Waiting for manual launch; press enter when ready...')
            return # Because the start happened manually.

        self.runner = subprocess.Popen([self.BASE_PATH + self.name])

        time.sleep(0.3) # Allow some time for runner to initialize...

    def stop(self):
        time.sleep(0.3) # Allow some time for runner to finish up...
...
class Connection(object):
    ...
    def _read(self):
        try:
            result = ''
            length = struct.calcsize(DATATYPES['size'])
